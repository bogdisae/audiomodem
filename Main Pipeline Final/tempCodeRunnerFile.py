...
def no_estimation_ofdm_interleaving_test():
    tx = Tx("filename.txt", constellation, big_shaq_data, repeatedChirp, golayPairs, whiteNoise, 2048, 4096, 10, 2_000, 12_000)
    tx.encode()
    original_data_symbols = tx.data_symbols
    sent_time_samples = tx.ofdm_symbol_blocks
    ...
    rx.ofdm_blocks = np.concatenate(sent_time_samples)
    rx.extract_ofdm_blocks()
    deinterleaved_data_symbols = rx.data_symbols

    for i in range(10000):
        t = np.array(original_data_symbols).flatten()[i]
        r = deinterleaved_data_symbols[i]
...
def full_transmission_test():
    ...
    # Are original and final bits correct
    # for i in range(1000):
    #     t = tx.data_bits[i]
    #     r = rx.data_bits[i]
    #     print(f"{'match' if t == r else 'error'} {t} : {r}")

full_transmission_test()