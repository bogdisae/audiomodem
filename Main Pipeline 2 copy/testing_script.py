...
def ask_sweep_spec() -> SweepSpec:
    sweep_param = questionary.select(
        ...
        choices=["TX chirp gap", "Golay pair gap", "Number of chirps", "Number of Golay pairs", "Length of Chirps", "Cyclic prefix length", "FFT length","Pilot spacing"],
    ).ask()
    ...
    default_name = {
        ...
        "Length of Chirps": "Chirp_length_sweep",
        "Cyclic prefix length": "CP_length_sweep",
        "FFT length": "FFT_length_sweep",
        "Pilot spacing": "Pilot_spacing_sweep",
    }[sweep_param]
...
def build_system(spec: SweepSpec, sweep_value: int):
    ...
    chirpLength = fixed["chirp_length"]
    ...
    elif spec.sweep_param == "Length of Chirps":
        chirpLength = sweep_value
    elif spec.sweep_param == "Cyclic prefix length":
        cp_length = sweep_value
    elif spec.sweep_param == "FFT length":
        block_length = sweep_value
    elif spec.sweep_param == "Pilot spacing":
        pilot_spacing = sweep_value
